...
def scale_boxes(bboxes, scale, image_shape=[1333, 1333]):
    ...
    # wanyu
    # avg_scale = np.zeros(bboxes.shape[0])
    # for i in range(bboxes.shape[0]):
    #     avg_scale[i] = np.mean(scale[np.round(bboxes[:, 1][i]).astype(int): np.round(bboxes[:, 3][i]).astype(int), np.round(bboxes[:, 0][i]).astype(int): np.round(bboxes[:, 2][i]).astype(int)])
    #     if math.isnan(avg_scale[i]):
    #         avg_scale[i] = 1.5
    # w_half *= avg_scale
    # h_half *= avg_scale
...
def ForegroundRegionScaleEqualization(bbox_list, foreground_region):
    ...
    for idx in range(len(foreground_region)):
        ...
        avg_area = np.mean(areas[inds])
        avg_areas[idx] = avg_area

    scale_factor = [1] * foreground_region.shape[0]
    for idx in range(foreground_region.shape[0]):
        if avg_areas[idx] < 96 * 96:
            scale_factor[idx] = math.floor(math.sqrt(96 * 96 / avg_areas[idx]))
...
def UnifiedForegroundPacking(bbox_list, scale, input_shape, output_shape=[1333,800]):
    ...
    # wanyu uncomment this to use scale equalization
