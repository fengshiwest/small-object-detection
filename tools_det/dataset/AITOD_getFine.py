...
def draw_ufp_result(results, img, img_name, w, h, anno_root=None):
    ...
    if anno_root:
        anno_path = os.path.join(anno_root, img_name.split('/')[0] + '_gt_ignore.txt')
        anno = open(anno_path)
        for rec in anno.readlines():
            rec = rec.strip()
            frame, target_id, x, y, _w, _h = [int(_) for _ in rec.split(',')[:6]]
            if img_name.split('img')[1][:-4].lstrip('0') == str(frame):
...
def main():
    ...
    for key in range(size):
        ...
        # show_result_pyplot(detector, img, first_results, score_thr=0.1)
        ...
        # ufp_img = draw_ufp_result(rec, img, img_name, w, h)
        if not os.path.exists(ufp_images_output_dir):
            os.makedirs(ufp_images_output_dir)
        sub_dir = os.path.join(ufp_images_output_dir, img_name.split('/')[0])
